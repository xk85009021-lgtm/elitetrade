...
盈透copy 是一个包含用户端、管理后台、资金审核、实名认证、跟单收益、推广关系和运营配置的全栈项目。当前版本已经完成 V1-V5 代理推广、JWT 用户鉴权、bcrypt 密码哈希、审核幂等事务、统一房间日收益率、新加坡时区 06:00 结算、7 天跟单退出锁、通知、客服、带单审核、审计日志、动态项目名称和 Logo 上传。
...
- 用户收益和止损权益真实计算。
- 邀请奖励、冻结钱包和解冻。
- V1-V5 五级代理体系：直推提成、小区手数奖、无限代级差奖、平级奖和满30天晋级奖。
- 众筹项目、股东群和群聊。
- 真实通知中心。
- 真实客服工单，后台可回复。
- 我的跟单、财务记录、个人资料和头像上传。
...
- 实名认证审核。
- 收益结算和手动结算。
- V1-V5 参数编辑、推广奖励、晋级奖、邀请奖励、团队业绩和推荐关系。
- 后台发布通知，支持用户当日首次登录弹窗提醒。
- 行情品种管理和实时加密行情刷新。
- 钱包地址、付款二维码和内容管理。
- 推荐关系和团队数据。
- 客服工单。
...
- 有效用户口径：注册 + KYC 通过 + 真实入金审核通过 + 非风控，不要求绑定 MT 账户。
...
- 客户打开充值页默认选择 USDT 和 TRC20。
- 客户可以随时切换币种和网络。
- 每次选择币种或网络后，系统从该网络对应地址池中随机分配一个钱包地址和二维码。
- 不再自动创建充值订单，不再显示15分钟倒计时。
- 充值 UID 和充值金额位于钱包地址下方、重要提示上方。
- 充值金额提供 100、500、2000、10000 四个快捷选项。
- 客户提交充值后才生成正式充值订单并进入后台审核。
- 充值提现订单明细可以查看审核状态，待审核订单可以取消后重新提交。
